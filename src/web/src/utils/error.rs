...
pub fn prepare_response<T>(result: Result<T, AppGenericError>) -> Result<Json<T>, AppResponseError> {
    match result {
        ...
        Err(generic_error) => match generic_error {
            AppGenericError::Auth(err) => Err(AppResponseError::new(StatusCode::UNAUTHORIZED, err.get_code(), err.get_message())),
            AppGenericError::Integration(err) => Err(AppResponseError::new(StatusCode::INTERNAL_SERVER_ERROR, err.get_code(), err.get_message())),
